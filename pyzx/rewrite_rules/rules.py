...
def apply_rule(
    g: BaseGraph[VT, ET],
    rewrite: Callable[
        [BaseGraph[VT, ET], List[MatchObject]], RewriteOutputType[VT, ET]
    ],
    m: List[MatchObject],
...
def match_bialg_parallel(
    ...
    if matchf is not None:
        candidates_set = set([e for e in g.edges() if matchf(e)])
    else:
        candidates_set = g.edge_set()
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if (
            ...
            if (
                all(
                    [types[n] == v1t and phases[n] == 0 for n in v0n]
                )  # all neighbors of v0 are of the same type as v1
                and all(
                    [types[n] == v0t and phases[n] == 0 for n in v1n]
                )  # all neighbors of v1 are of the same type as v0
                and g.num_edges(v0, v1)
                == 1  # there is exactly one edge between v0 and v1
                and g.num_edges(v0, v0) == 0  # there are no self-loops on v0
                and g.num_edges(v1, v1) == 0
            ):  # there are no self-loops on v1
                i += 1
                for vn in [v0n, v1n]:
                    for v in vn:
    ...
    for v in list(candidates):
        ...
        for e in g.incident_edges(v):
            ...
            if (
                ...
                or v0 in used
                or v1 in used
                or v0 not in candidates
                or v1 not in candidates
            ):
                continue
...
def match_pivot_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        v0a = phases[v0]
        v1a = phases[v1]
        if not (phase_is_pauli(v0a) and phase_is_pauli(v1a)):
            continue
        if g.is_ground(v0) or g.is_ground(v1):
            continue
    ...
    for m in matches:
        ...
        for i in 0, 1:
            ...
            if not m[1][i]:
                ...
                if g.edge_type(g.edge(e[0], e[1])) == EdgeType.SIMPLE:
                    nhe += 1
                elif g.edge_type(g.edge(e[0], e[1])) == EdgeType.HADAMARD:
                    ne += 1
                etab[new_e] = [ne, nhe]
...
def match_lcomp_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if vt != VertexType.Z:
            continue
        if not phase_is_clifford(va) or phase_is_pauli(va):
            continue
...
def match_ids_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if (
            phases[v] != 0
            or not vertex_is_zx(types[v])
            or g.is_ground(v)
            or g.vertex_degree(v) != 2
        ):
            continue
        if len(g.incident_edges(v)) != 2:
            continue
        neigh = g.neighbors(v)
        if len(neigh) == 2:
            v0, v1 = neigh
...
def match_hopf(
    ...
    for v in candidates:
        if not vertex_is_zx_like(types[v]):
            continue
        for w in g.neighbors(v):
            if not w in candidates or not vertex_is_zx_like(types[w]):
                continue
...
def hopf(
    g: BaseGraph[VT, ET], matches: List[MatchHopfType[VT]]
) -> RewriteOutputType[VT, ET]:
...
def remove_self_loops(
    g: BaseGraph[VT, ET], matches: List[MatchSelfLoopType[VT]]
) -> RewriteOutputType[VT, ET]:
...
def match_phase_gadgets(
    g: BaseGraph[VT, ET], vertexf: Optional[Callable[[VT], bool]] = None
) -> List[MatchGadgetType[VT]]:
    ...
    for v in candidates:
        ...
        if non_clifford and len(list(g.neighbors(v))) == 1:
            ...
            if not phase_is_pauli(phases[n]):
            ...
            if par == frozenset():
                continue  # Not a real phase gadget if it acts on nothing
            if par in parities:
...
def match_supplementarity(
    g: BaseGraph[VT, ET], vertexf: Optional[Callable[[VT], bool]] = None
) -> List[MatchSupplementarityType[VT]]:
    ...
    while len(candidates) > 0:
        ...
        if par in parities:
            ...
            parities[par] = [v]
        for w in neigh:
            if phase_is_clifford(phases[w]) or w in taken:
                continue
            ...
            for j in range(i + 1, 4):
                ...
                for k in range(j + 1, 4):
                    ...
                    if f in gadgets:
                        ...
                        phase = 0
                        g.add_edges(
                            [(n, v), (v1, n), (v2, n), (v3, n)], EdgeType.HADAMARD
                        )
